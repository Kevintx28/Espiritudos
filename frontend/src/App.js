...
function App() {
  ...
  const makeOrder = (nextPayment) => { const nextOrder = { id: orderCode(), date: new Date().toLocaleString('es-PE'), country, game, items, total, form, paymentMethod: nextPayment.paymentMethod, testOnly: items.some(({ product }) => product.testOnly) }; setPayment(nextPayment); setOrder(nextOrder); saveDraft(form, nextPayment); setStep('receipt'); };
  const steps = ['Catálogo', 'Carrito', 'Instrucciones', 'Datos', 'Pago', 'Comprobante'];
  return <div className="App min-h-screen text-slate-100"><header className="sticky top-0 z-20 border-b border-white/10 bg-[#09090b]/90 px-4 py-4 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl flex-col gap-4"><div className="flex items-center justify-between gap-4"><div><div className="text-2xl font-black tracking-tight text-gradient">{store.identity.name}</div><p className="text-xs text-slate-400">{store.identity.tagline}</p></div><button type="button" onClick={() => setCountryOpen(true)} className="flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-sm text-slate-200"><Globe className="h-4 w-4 text-cyan-300" />{country?.flag} {country?.name}</button></div><GameNavigation games={store.games} activeGame={gameId} onSelect={(next) => { setGameId(next); setStep('catalog'); }} /><div className="flex gap-2 overflow-x-auto text-xs text-slate-500">{steps.map((label, index) => <span key={label} className={step === ['catalog', 'cart', 'tutorial', 'form', 'payment', 'receipt'][index] ? 'font-bold text-cyan-300' : ''}>{index + 1}. {label}</span>)}</div></div></header><main className="mx-auto max-w-7xl px-4 py-8">{step === 'catalog' && <><section className="mb-8 max-w-2xl"><p className="mb-3 text-sm font-bold uppercase tracking-[0.25em] text-pink-300">Gaming store / entrega clara</p><h1 className="text-4xl font-black text-white md:text-6xl">Compra para jugar. <span className="text-gradient">Sin credenciales.</span></h1><p className="mt-4 text-slate-400">Códigos, recargas y coordinación manual. Validamos cada pedido antes de entregarlo.</p></section><div className="grid gap-8 lg:grid-cols-[1fr_340px]"><StoreCatalog game={game} products={products} onAdd={addProduct} /><GameCart items={items} onChange={changeQuantity} onClear={clearCart} onContinue={() => setStep('tutorial')} /></div></>}{step === 'tutorial' && <RedeemTutorial game={game} onBack={() => setStep('catalog')} onContinue={() => setStep('form')} />}{step === 'form' && <DynamicOrderForm game={game} initialValue={form} onBack={() => setStep('tutorial')} onContinue={(next) => { setForm(next); saveDraft(next, payment); setStep('payment'); }} />}{step === 'payment' && <PaymentVoucher country={country} game={game} items={items} form={form} initialValue={payment} onBack={() => setStep('form')} onContinue={makeOrder} />}{step === 'receipt' && order && <><OrderReceiptGenerator order={order} onGenerated={setImageUrl} /><div className="mx-auto max-w-2xl"><p className="mb-4 text-center text-sm text-slate-400">Generando comprobante local...</p>{imageUrl && <OrderCompletion order={order} imageUrl={imageUrl} onFinish={finish} />}</div></>}{step === 'cart' && <GameCart items={items} onChange={changeQuantity} onClear={clearCart} onContinue={() => setStep('tutorial')} />}</main>{countryOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4"><div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#15131f] p-6"><h2 className="text-2xl font-bold text-white">Selecciona tu país</h2><p className="mt-2 text-sm text-slate-400">Los precios se muestran en PEN/S/. Para otros países de LatAm, consulta métodos disponibles por WhatsApp.</p><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">{store.countries.map((item) => <button key={item.code} type="button" data-testid={`country-${item.code}`} onClick={() => selectCountry(item)} className={`rounded-xl border p-4 text-left ${country?.code === item.code ? 'border-cyan-400 bg-cyan-400/10' : 'border-white/10'}`}><span className="text-2xl">{item.flag}</span><span className="mt-2 block text-sm font-bold text-white">{item.name}</span></button>)}</div><button type="button" onClick={() => setCountryOpen(false)} className="mt-5 text-sm text-slate-400 underline">Cerrar</button></div></div>}</div>;
}
export default App;
