...
function App() {
  ...
  const [fortniteSubcategory, setFortniteSubcategory] = useState('gift-shop');
  ...
  return <div className="App min-h-screen text-slate-100">
    ...
    <main className={`mx-auto max-w-7xl px-4 py-8 ${itemCount ? 'pb-28 sm:pb-8' : ''}`}>
      {step === 'catalog' && <><section className="mb-8 max-w-2xl"><p className="mb-3 text-sm font-bold uppercase tracking-[0.25em] text-pink-300">Gaming store / entrega clara</p><h1 className="text-4xl font-black text-white md:text-6xl">Compra para jugar. <span className="text-gradient">Sin credenciales.</span></h1><p className="mt-4 text-slate-400">Códigos, recargas y coordinación manual. Validamos cada pedido antes de entregarlo.</p></section><div className={gameId === 'fortnite' ? '' : 'grid gap-8 lg:grid-cols-[1fr_340px]'}><StoreCatalog key={gameId} game={game} products={products} fortniteSubcategory={fortniteSubcategory} onAdd={addProduct} />{gameId !== 'fortnite' && <GameCart items={items} calculations={calculations} onChange={changeQuantity} onClear={clearCart} onContinue={continueCheckout} />}</div></>}
