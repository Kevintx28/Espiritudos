...
function App() {
  ...
  return <div className="App min-h-screen text-slate-100">
    ...
    <main className={`mx-auto max-w-7xl px-4 py-8 ${itemCount ? 'pb-28 sm:pb-8' : ''}`}>
      ...
      {step === 'cart' && <GameCart items={items} onChange={changeQuantity} onClear={clearCart} onContinue={continueCheckout} />}
    </main>
    {itemCount > 0 && <FloatingCart itemCount={itemCount} total={total} onOpen={() => setCartOpen(true)} />}
    {cartOpen && <CartDrawer items={items} total={total} onChange={changeQuantity} onClear={clearCart} onContinue={continueCheckout} onClose={() => setCartOpen(false)} />}
