...
export default function StoreCatalog({ game, products, onAdd }) {
  ...
  return <section aria-labelledby="catalog-title"><div className="mb-5"><p className="text-sm uppercase tracking-[0.2em] text-cyan-300">Catálogo</p><h2 id="catalog-title" className="text-2xl font-bold text-white">{game.name}</h2></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{products.map((product) => <ProductCard key={product.id} product={product} onAdd={onAdd} />)}</div></section>;
}
