...
import { Gamepad2, Crosshair, Flame, Target, Shield, Sparkles, House } from 'lucide-react';

const icons = { Gamepad2, Crosshair, Flame, Target, Shield, Sparkles };

export default function GameNavigation({ games, activeGame, onSelect, onHome, homeActive = false }) {
  return <nav aria-label="Juegos" className="flex gap-2 overflow-x-auto pb-2" data-testid="game-navigation">
    <button type="button" data-testid="game-home" onClick={onHome} className={`flex min-w-max items-center gap-2 rounded-xl border px-4 py-3 text-sm font-bold transition ${homeActive ? 'border-cyan-400 bg-cyan-400/15 text-cyan-200' : 'border-white/10 bg-white/5 text-slate-300 hover:border-pink-400/60'}`}><House className="h-4 w-4" />Inicio</button>
    {games.map((game) => {
